...
console.log('Selected month March:', paymentMap);

// What if selected Month is April ?
paymentMap = {};
const selectedMonth2 = '2026-04';
records.forEach(r => {
    const pm = r.payment_method || 'SIN ESPECIFICAR';
    if (!paymentMap[pm]) {
        paymentMap[pm] = { initial: 0, income: 0, expense: 0, finalBalance: 0 };
    ...
    if (selectedMonth2 !== 'all' && recordMonth < selectedMonth2) {
        if (isInitialBalance) {
            paymentMap[pm].finalBalance = recordIncome - recordExpense; 
        } else {
            paymentMap[pm].finalBalance += recordIncome - recordExpense; 
    ...
    } else if (selectedMonth2 === 'all' || recordMonth === selectedMonth2) {
        // ...
    }
});
console.log('Selected month April:', paymentMap);
